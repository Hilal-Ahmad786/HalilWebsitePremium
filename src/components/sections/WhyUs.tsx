        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, index) => (
            <div
              ...
              className={`group relative bg-gradient-to-br from-gray-50 to-white rounded-2xl p-6 border-2 transition-all duration-300 cursor-pointer ${hoveredIndex === index
            ...
            >
              {/* Top Accent Line */}
              <div
                className={`absolute top-0 left-0 right-0 h-1 rounded-t-2xl bg-gradient-to-r ${colors[index % colors.length]} transition-all ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'
                  }`}
              />

              {/* Icon */}
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-r ${colors[index % colors.length]} text-white text-3xl mb-4 shadow-lg ${hoveredIndex === index ? 'scale-110 rotate-6' : ''
                } transition-all duration-300`}>
                <i className={item.icon} />
              </div>

              {/* Content */}
              <h3 className={`text-xl font-bold mb-3 ${hoveredIndex === index ? 'text-turuncu-600' : 'text-gray-900'
                } transition-colors`}>
                {item.title}
              </h3>
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-r ${colors[index % colors.length]} opacity-0 ${hoveredIndex === index ? 'group-hover:opacity-5' : ''
