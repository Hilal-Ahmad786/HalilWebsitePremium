...
export default function ServicePageTemplate({ service, otherServices }: { service: Service; otherServices: Service[] }) {
  ...
  return <>
    ...
    {service.scenarios && service.scenarios.length > 0 && (
      <section className="border-y border-line bg-surface"><div className="site-container section-space"><p className="eyebrow">SIK KARŞILAŞILAN DURUMLAR</p><h2 className="display mt-4 text-4xl">Aracınızın durumu bunlardan biri olabilir</h2><div className="mt-10 grid gap-5 md:grid-cols-3">{service.scenarios.map((item) => <div key={item.title} className="border border-line bg-bg p-5"><h3 className="display text-lg leading-snug">{item.title}</h3><p className="mt-3 text-sm leading-6 text-muted">{item.description}</p></div>)}</div></div></section>
    )}

    <section className="site-container section-space"><p className="eyebrow">SIK SORULAN SORULAR</p><h2 className="display mt-4 text-4xl">Merak edilenler</h2><div className="mt-8 divide-y divide-line border-y border-line">{serviceFaqs.map((faq) => <details key={faq.question} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-6 font-bold text-ink">{faq.question}<span aria-hidden="true" className="text-lg transition group-open:rotate-45">+</span></summary><p className="max-w-3xl pt-4 leading-7 text-muted">{faq.answer}</p></details>)}</div></section>

    <section className="border-y border-line bg-surface"><div className="site-container section-space"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">HİZMET BÖLGELERİ</p><h2 className="display mt-4 text-4xl">Türkiye genelinde araç alımı</h2><p className="mt-5 max-w-lg leading-8 text-muted">Bulunduğunuz şehirdeki araç alım hizmeti hakkında bilgi almak için ilgili şehir sayfasını inceleyebilir veya doğrudan bize ulaşabilirsiniz.</p></div></div><div className="mt-10 grid grid-cols-2 border-t border-line sm:grid-cols-3 lg:grid-cols-5">{publishedCities.map((city) => <Link key={city.slug} href={`/${city.slug}`} className="flex items-center gap-2 border-b border-line py-4 text-sm font-semibold text-muted hover:text-ink"><FaMapMarkerAlt className="text-xs" />{city.name}</Link>)}</div></div></section>

    <section className="site-container section-space"><p className="eyebrow">İLGİLİ HİZMETLER</p><div className="mt-7 grid gap-3 md:grid-cols-3">{related.map((item) => <Link key={item.id} href={`/hizmetler/${item.slug}`} className="group flex items-center justify-between border border-line bg-bg p-5"><div><h2 className="display text-lg">{item.title}</h2><p className="mt-2 text-sm text-muted">{item.shortDescription}</p></div><FaArrowRight className="shrink-0 transition group-hover:translate-x-1" /></Link>)}</div></section>
