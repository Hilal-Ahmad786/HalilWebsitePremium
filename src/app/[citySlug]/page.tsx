...
import { FaArrowRight, FaCheck, FaPhoneAlt, FaShieldAlt, FaWhatsapp } from 'react-icons/fa';
...
const cityImages: Record<string, string> = {
  istanbul: '/Newimages/accident/damaged-front.jpg', ankara: '/Newimages/accident/damaged-side.jpg',
  izmir: '/Newimages/accident/accident-detail.jpg', bursa: '/Newimages/accident/wrecked-vehicle.jpg',
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return city ? {
    ...
    openGraph: { title: city.metaTitle, description: city.metaDescription, url: `/${city.slug}`, type: 'website', images: [{ url: cityImages[city.id] || '/Newimages/accident/damaged-front.jpg', width: 1800, height: 1200, alt: city.hero.title }] },
...
export default async function CityPage({ params }: Props) {
  ...
  const city = cities.find((item) => item.slug === citySlug && item.published);
  if (!city) notFound();
  const image = cityImages[city.id] || '/Newimages/accident/damaged-front.jpg';
  const cityFaqs = [
  ...
  return <>
    ...
    <section className="site-container section-space grid gap-10 lg:grid-cols-[.85fr_1.15fr]"><div><p className="eyebrow">{city.name.toLocaleUpperCase('tr-TR')} HİZMET AĞI</p><h2 className="display mt-4 text-4xl leading-tight">{city.intro.title}</h2><p className="mt-5 leading-8 text-muted">Şehrinizdeki aracın durumunu uzaktan ön değerlendirme ile paylaşabilir, gerekli olduğunda yerinde inceleme için süreç hakkında bilgi alabilirsiniz.</p><div className="mt-8 grid gap-4">{city.intro.highlights.map((item) => <div key={item} className="flex gap-3 border-t border-line py-4"><FaCheck className="mt-1 shrink-0" /><p className="text-muted">{item}</p></div>)}</div></div><div className="relative min-h-[330px] overflow-hidden rounded-2xl bg-soft"><Image src={image} alt={`${city.name} hasarlı araç alımı`} fill sizes="(max-width:1024px) 100vw,50vw" className="object-cover" /></div></section>
